...
const BookingBar = ({ listingId, basePricePerNight = null, minStayNights = 1 }: BookingBarProps) => {
  ...
  const canReserve = !isReserveDisabled && !!listingId;
      ...
      <div ref={containerRef} className="w-full max-w-[1050px] pointer-events-auto relative">
          ...
          {activePopup === 'calendar' && (
            ...
            >
              <div className="flex justify-between items-center mb-6 md:mb-8 px-2">
                <span className="text-[11px] font-bold text-gray-400 uppercase tracking-widest">Choose check-in date</span>
              ...
              {/* Month Selector */}
              <div className="flex justify-between items-center mb-6 md:mb-8 px-2">
                <button onClick={() => setViewDate(subMonths(viewDate, 1))}><ChevronLeft size={24} /></button>
                <span className="text-lg md:text-xl font-bold text-gray-900">{format(viewDate, 'MMMM yyyy')}</span>
                <button onClick={() => setViewDate(addMonths(viewDate, 1))}><ChevronRight size={24} /></button>
              </div>
          ...
          {activePopup === 'guests' && (
            ...
            >
              <div className="flex justify-between items-center mb-8 md:mb-10">
                <span className="text-sm text-gray-400 font-bold">Choose guest numbers (up to 8)</span>
                <button onClick={() => setActivePopup(null)} className="p-1 border-2 border-black rounded-lg"><X size={16} strokeWidth={3} /></button>
              </div>
              <div className="space-y-6 md:space-y-8">
                <GuestRow label="Adults" sub="Age 13+" value={guests.adults} onInc={() => setGuests({ ...guests, adults: guests.adults + 1 })} onDec={() => setGuests({ ...guests, adults: Math.max(1, guests.adults - 1) })} />
                <GuestRow label="Childrens" sub="Age 2-12" value={guests.children} onInc={() => setGuests({ ...guests, children: guests.children + 1 })} onDec={() => setGuests({ ...guests, children: Math.max(0, guests.children - 1) })} />
        ...
        <div className={`bg-white md:rounded-full shadow-[0_12px_40px_rgba(0,0,0,0.12)] border-t md:border border-gray-100 p-2 md:pl-12 px-4 flex flex-col md:flex-row items-center justify-between h-auto md:h-[96px] w-full gap-2 md:gap-0 transition-all duration-300 ${isLoginModalOpen ? 'opacity-40 pointer-events-none filter grayscale-[0.5]' : 'opacity-100'}`}>
          ...
          <div className="flex md:hidden w-full items-center justify-between px-6 pt-2 pb-0.5">
            ...
            <button onClick={() => setActivePopup('calendar')} className="text-xs font-bold underline decoration-zinc-300 underline-offset-4">Change dates</button>
          </div>
          ...
          <div className="flex flex-1 md:flex-none items-center py-2 w-[94%] md:w-auto h-14 md:h-full border border-gray-100 rounded-2xl md:rounded-[30px] bg-white overflow-hidden shadow-sm md:shadow-none">
            <button onClick={() => setActivePopup('calendar')} className="flex flex-col flex-1 px-4 md:px-10 text-left hover:bg-gray-50 h-full justify-center border-r border-gray-100 min-w-0 md:min-w-[170px]">
...
// Row Sub-component
const GuestRow = ({ label, sub, value, onDec, onInc, isPet }: any) => (
